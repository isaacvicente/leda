...
public class RepositorioProdutosArray<T extends Produto> implements RepositorioProdutos<T> {
	...
	public void inserir(T produto) {
        if (index + 1 <= this.produtos.length - 1) {
	...
	public void atualizar(T produto) {
        ...
        if (existe(codigo)) {
            this.produtos[procurarIndice(codigo)] = produto;
        } else {
            throw new RuntimeException("Produto não pode ser atualizado pois não existe");
        }
	}
	...
	public void remover(int codigo) {
		...
		if (i != -1) {
			...
			throw new RuntimeException("Não foi possível remover, pois produto não existe");
		}
	}
